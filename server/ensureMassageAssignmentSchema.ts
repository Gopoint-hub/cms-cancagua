...
const columnDefinitions = [
  ...
  "ALTER TABLE `massage_program_bookings` ADD COLUMN `booking_group_id` varchar(36) NULL AFTER `id`",
  "ALTER TABLE `massage_program_bookings` ADD COLUMN `group_sequence` int NOT NULL DEFAULT 1 AFTER `booking_group_id`",
  "ALTER TABLE `massage_program_bookings` ADD COLUMN `group_size` int NOT NULL DEFAULT 1 AFTER `group_sequence`",
  "ALTER TABLE `massage_program_bookings` ADD COLUMN `schedule_mode` enum('simultaneous','two_by_two') NOT NULL DEFAULT 'simultaneous' AFTER `group_size`",
] as const;
...
export async function ensureMassageAssignmentSchema(): Promise<void> {
  ...
  if (!db)
    throw new Error(
      "Database unavailable while ensuring therapist assignment schema"
    );
  ...
  try {
    await db.execute(
      sql.raw(`
      CREATE INDEX \`massage_program_booking_group_idx\`
      ON \`massage_program_bookings\` (\`booking_group_id\`, \`group_sequence\`)
    `)
    );
  } catch (error) {
    const candidate = error as {
    ...
    const duplicateIndex =
      candidate.code === "ER_DUP_KEYNAME" ||
      candidate.errno === 1061 ||
      candidate.cause?.code === "ER_DUP_KEYNAME" ||
      candidate.cause?.errno === 1061;
