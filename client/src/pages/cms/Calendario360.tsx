    ...
    <DashboardLayout>
      <div className="space-y-5 p-2 sm:p-4">
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">{canCreateReservation && <Button className="gap-2" onClick={() => setBookingOpen(true)}><Plus className="h-4 w-4" />Nueva reserva</Button>}<div className="flex rounded-lg border bg-background p-1">{(["day", "week", "month"] as ViewMode[]).map(mode => <Button key={mode} size="sm" variant={view === mode ? "default" : "ghost"} onClick={() => setView(mode)}>{mode === "day" ? "Día" : mode === "week" ? "Semana" : "Mes"}</Button>)}</div></div>
        </div>
