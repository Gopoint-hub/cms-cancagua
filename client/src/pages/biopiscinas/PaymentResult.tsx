...
export default function BiopoolPaymentResult() {
  ...
  return <main className="min-h-screen bg-[#f5f1e9] px-4 py-12 grid place-items-center"><Card className="w-full max-w-xl border-0 shadow-xl"><CardContent className="p-8 text-center space-y-5">
    {query.isLoading ? <><Clock3 className="mx-auto h-14 w-14 animate-pulse text-[#536481]" /><h1 className="font-serif text-3xl">Confirmando tu pago…</h1></> : paid ? <><CheckCircle2 className="mx-auto h-16 w-16 text-emerald-600" /><h1 className="font-serif text-3xl">¡Reserva confirmada!</h1><p className="text-stone-600">Tu pago de {clp.format(query.data?.totalClp ?? 0)} fue aprobado. Enviamos la confirmación a {query.data?.clientEmail}.</p><div className="rounded-xl bg-emerald-50 p-4 text-emerald-900"><strong>Reserva {query.data?.bookingCode}</strong><br />{query.data?.date} · ingreso {query.data?.startTime}</div></> : pending ? <><Clock3 className="mx-auto h-16 w-16 text-amber-600" /><h1 className="font-serif text-3xl">Estamos verificando tu pago</h1><p className="text-stone-600">No cierres esta página. La confirmación puede tardar unos segundos.</p></> : <><XCircle className="mx-auto h-16 w-16 text-red-600" /><h1 className="font-serif text-3xl">El pago no se completó</h1><p className="text-stone-600">No se generó una reserva ni se realizó un cobro confirmado. Puedes intentarlo nuevamente.</p></>}
